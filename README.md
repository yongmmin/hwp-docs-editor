...
4. **저장** — `Cmd+S` / `Ctrl+S` 로 현재 편집 내용을 문서 스냅샷에 저장
5. **내보내기** — 헤더 `내보내기` 버튼 → 편집 내용이 반영된 `.hwp` / `.hwpx` 다운로드 (원본 포맷 유지)

   > ⚠️ 내보내기 파이프라인은 1차 구현 완료 상태이지만 실 파일 검증에서 **편집 내용이 반영되지 않는 현상**이 남아 있습니다. 상세 내용과 디버깅 계획: [docs/EXPORT_REWRITE_PLAN.md](docs/EXPORT_REWRITE_PLAN.md)
...
| HWP / HWPX 내보내기 | 원본 바이트 유지 + 편집 단락 in-place 패치 (재작성 진행 중) |
...
- [로드맵](docs/ROADMAP.md)
- [내보내기 재작성 계획](docs/EXPORT_REWRITE_PLAN.md)
